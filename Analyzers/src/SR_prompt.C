...
void SR_prompt::initializeAnalyzer(){
...
void SR_prompt::Select_syst_objects(AnalyzerParameter param){
...
void SR_prompt::executeEventFromParameter(AnalyzerParameter param, std::vector<Electron> electron_all, std::vector<Muon> muons_all, std::vector<Jet> jets_all, std::vector<FatJet> fatjets_all, TString syst_flag, double PDF_weight){
  ...
  std::vector<Electron> electrons_Veto = Get_nonCF_Electrons(ElectronPromptOnly(SelectElectrons(electron_all, param.Electron_Veto_ID, 35., 2.5), gens), gens);
  std::vector<Electron> electrons_Loose = Get_nonCF_Electrons(ElectronPromptOnly(SelectElectrons(electron_all, param.Electron_Loose_ID, param.Electron_MinPt, 2.5), gens), gens);
  std::vector<Electron> electrons_Tight = Get_nonCF_Electrons(ElectronPromptOnly(SelectElectrons(electron_all, param.Electron_Tight_ID, param.Electron_MinPt, 2.5), gens), gens);
  std::vector<Electron> electrons_Tight_all = Get_nonCF_Electrons(ElectronPromptOnly(SelectElectrons(electron_all, param.Electron_Tight_ID, param.Electron_MinPt, 2.5), gens), gens);
  
  // -- Save Muons and Numbers of them
  std::vector<Muon> muons_Veto  = MuonPromptOnly(SelectMuons(muons_all, param.Muon_Veto_ID,  10., 2.4), gens);
  std::vector<Muon> muons_Loose = MuonPromptOnly(SelectMuons(muons_all, param.Muon_Loose_ID, 10., 2.4), gens);
  std::vector<Muon> muons_Tight = MuonPromptOnly(SelectMuons(muons_all, param.Muon_Tight_ID, 10., 2.4), gens);
  std::vector<Muon> muons_Tight_all = MuonPromptOnly(SelectMuons(muons_all, param.Muon_Tight_ID, 10., 2.4), gens);
...
void SR_prompt::SR(AnalyzerParameter param, TString channel, bool trig_pass, double weight, std::vector<Jet> jets, std::vector<FatJet> fatjets, std::vector<Electron> electrons, std::vector<Electron> electrons_veto, std::vector<Muon> muons, std::vector<Muon> muons_veto,
...
void SR_prompt::CR_inv_mll(AnalyzerParameter param, TString channel, bool trig_pass, double weight, std::vector<Jet> jets, std::vector<FatJet> fatjets, std::vector<Electron> electrons, std::vector<Electron> electrons_veto, std::vector<Muon> muons, std::vector<Muon> muons_veto,
		     int N_electron, int N_veto_ele, int N_muon, int N_veto_muon){
...
vector<Particle> SR_prompt::RecoPairN(vector<Lepton *> lepptrs, vector<FatJet> fatjets, vector<Jet> jets){
...
TString SR_prompt::Get_N_jet_bin(vector<Lepton *> lepptrs, vector<FatJet> fatjets, vector<Jet> jets){
...
std::vector<Electron> SR_prompt::Get_nonCF_Electrons(std::vector<Electron> electrons, vector<Gen> gens){
  
  std::vector<Electron> out;
  for(unsigned int i = 0; i < electrons.size(); i++){
    Electron this_electron = electrons.at(i);
    
    Gen genlep = GetGenMatchedLepton(this_electron, gens);
    if(genlep.IsEmpty()) continue;
    bool IsCF = this_electron.Charge()!=genlep.Charge();
    if(!IsCF) out.push_back(this_electron);
  }
